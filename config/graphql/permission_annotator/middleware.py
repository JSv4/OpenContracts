...
def get_permissions_for_user_on_model_in_app(app_name, model_name, info):
    # logger.info(
    ...
    try:
        ...
        if context and hasattr(context, "user"):
            user = context.user
            # logger.info(f"get_user_model_permissions_from_info_and_model - User on the context: {user}")
        ...
        logger.error(
            f"Error getting object-level permissions based on {info} and with "
...
class PermissionAnnotatingMiddleware:
    ...
    def resolve(self, next, root, info, **kwargs):
        ...
        try:
            ...
            logger.error(
                f"PermissionAnnotatingMiddleware - could not determine the graphene_type due to error: {e}"
            )
