...
class AnnotatePermissionsForReadMixin:
    ...
    def resolve_my_permissions(self, info):
        ...
        # start to request nested objs WITH permissions (as we do in OpenContractsServer in some cases).
        ...
        # logger.info(f"resolve_my_permissions() - permission_annotations: {permission_annotations}")
        ...
        try:
            ...
            if user:
                ...
                try:
                    ...
                    # Super user won't have explicit rights in the permission object set YET
                    # super users have ALL permissions available in django. If user is making
                    # request, annotate all permissions for user
                    if user.is_superuser:
                        # logger.info(f"resolve_my_permissions() - ")
                        # logger.info(f"resolve_my_permissions() - permissions values:
                        # {permission_annotations[full_name]['this_model_permission_id_map'].values()}")
                        permissions = {
                            "superuser": "superuser",
                        }
                        if (
                        ...
                        # logger.info(f"resolve_my_permissions() - permissions: {permissions}")
                    else:
                        # logger.info("resolve_my_permissions() - user is not super user.")
                        if full_name not in permission_annotations:
                            ...
                            # TODO - manual lookup here from database
                        else:
                            # logger.info(f"resolve_my_permissions() - {full_name} is in permission_annoations")
                            model_permissions = permission_annotations[full_name]
                            # logger.info(f"resolve_my_permissions() - model_name: {model_name}")

                            # GET PERMISSION IDS FOR MODEL ####
                            this_user_group_ids = model_permissions.get(
                                "this_user_group_ids", []
                            )
                            # logger.info(f"resolve_my_permissions() - this_user_group_ids: {this_user_group_ids}")
                            ...
                            # logger.info(f"resolve_my_permissions() - can_publish_model_type:
                            # {can_publish_model_type}")
                            #####################################################################

                            this_user_perms = getattr(
                                self, f"{model_name}userobjectpermission_set"
                            )
                            # logger.info(f"resolve_my_permissions() - this_user_perms: {this_user_perms}")

                            this_user_perms = this_user_perms.filter(user_id=user.id)
                            # logger.info(f"resolve_my_permissions() - filtered this_user_perms: {this_user_perms}")

                            this_users_group_perms = getattr(
                                self, f"{model_name}groupobjectpermission_set"
                            ).filter(group_id__in=this_user_group_ids)
                            # logger.info(f"resolve_my_permissions() - this_users_group_perms:
                            # {this_users_group_perms}")

                            # logger.info(f"resolve_my_permissions() - Analyze this_user_perms")
