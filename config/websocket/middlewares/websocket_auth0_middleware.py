...
class MockWSRequest(HttpRequest):
    ...
    def __init__(self, *, token: str = "", headers: Optional[List[Tuple[bytes, bytes]]] = None) -> None:
        super().__init__()
        self.method = "GET"
...
class WebsocketAuth0TokenMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        logger.info(f"Found {len(headers)} headers in request")
        logger.info(f"Authorization header present: {'Yes' if any(h[0].lower() == b"authorization" for h in headers) else 'No'}")

        # 2. If we found a token or relevant header, try to authenticate
        if token or any(h[0].lower() == b"authorization" for h in headers):
            logger.info("Attempting authentication with provided credentials")
            try:
                request = MockWSRequest(
                    token=token if token else "",
                    headers=headers if headers else None,
                )
                logger.info("Created mock request object for authentication")

                user = database_sync_to_async(authenticate)(request=request)
                if user and isinstance(user, User):
                    logger.debug(f"Websocket user authenticated: {user.username}")
                    logger.info(f"Successfully authenticated user {user.username} with ID {user.id}")
                    scope["user"] = user
                else:
                    logger.warning("Websocket token authentication failed, using AnonymousUser")
                    logger.info("Authentication attempt returned no valid user")

            except Exception as e:
                logger.error(f"Error during Websocket auth: {e}", exc_info=True)
                logger.info("Authentication attempt failed with exception")
