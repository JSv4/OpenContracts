...
export const SelectionBoundary = ({
  ...
  const [handledScroll, setHandledScroll] = useState(false);
  ...
  const createRefAndScrollIfPreSelected = (element: HTMLSpanElement | null) => {
    if (element && selectionRef && id) {
      ...
      if (scrollIntoView && !handledScroll) {
        element.scrollIntoView();
        setHandledScroll(true);
...
const EditLabelModal = ({
  ...
  const dropdownOptions: DropdownItemProps[] = annotationStore.labels.map(
    (label, index) => ({
      key: label.id,
      text: label.text,
      value: label.id,
  ...
  const handleDropdownChange = (
    ...
    const label = annotationStore.labels.find((l) => l.id === data.value);
    if (!label) {
      return;
    }
