...
import { PDF, RelationModal, sidebarWidth, WithSidebar } from "..";
import {
  BoundingBox,
  PermissionTypes,
...
export const PDFView = ({
  ...
  const [activeLabel, setActiveLabel] = useState<AnnotationLabelType>(
    labels[0]
  );
        ...
        >
          ...
          <WithSidebar width={sidebarWidth}>
            {corpus_permissions.includes(PermissionTypes.CAN_UPDATE) ? (
              <LabelSelector sidebarWidth={sidebarWidth} />
            ) : (
              <></>
            )}
            <SidebarContainer width={sidebarWidth}>
              <AnnotatorSidebar read_only={false} />
            </SidebarContainer>
