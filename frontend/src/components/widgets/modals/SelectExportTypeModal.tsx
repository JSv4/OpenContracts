...
import { Button, Dropdown, DropdownProps, Modal, Message, Divider, Header } from "semantic-ui-react";
import { exportingCorpus } from "../../../graphql/cache";
import { StartExportCorpusInputs, StartExportCorpusOutputs, START_EXPORT_CORPUS } from "../../../graphql/mutations";
...
export function SelectExportTypeModal({
  ...
  const [exportFormat, setExportFormat] = useState<ExportTypes>(ExportTypes.OPEN_CONTRACTS);
  ...
  const triggerCorpusExport = () => {
    if (exporting_corpus){
      startExportCorpus({ variables: { corpusId: exporting_corpus?.id, exportFormat } });
    }
  };
  ...
  const handleDropdownChange = (
    ...
    console.log("Handle export type select dropdown selection", data.value, typeof data.value);
