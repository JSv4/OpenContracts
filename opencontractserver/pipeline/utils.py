...
def get_all_thumbnailers() -> List[Type[BaseThumbnailGenerator]]:
    ...
    """
    return get_all_subclasses('opencontractserver.pipeline.thumbnailers', BaseThumbnailGenerator)
...
def get_component_by_name(component_name: str) -> Type:
    ...
    """
    try:
        module = importlib.import_module(f"opencontractserver.pipeline.parsers.{component_name}")
        for name, obj in inspect.getmembers(module, inspect.isclass):
    ...
    try:
        module = importlib.import_module(f"opencontractserver.pipeline.embedders.{component_name}")
        for name, obj in inspect.getmembers(module, inspect.isclass):
    ...
    try:
        module = importlib.import_module(f"opencontractserver.pipeline.thumbnailers.{component_name}")
        for name, obj in inspect.getmembers(module, inspect.isclass):
