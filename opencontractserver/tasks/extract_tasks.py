        ...
        for column in fieldset.columns.all():
            ...
            try:
                ...
                if column.limit_to_label:
                    annotations = annotations.filter(annotation_label__text=column.limit_to_label)

                match_text = column.match_text or column.query

                if match_text:
                    # Find closest annotations
                    annotations = annotations.order_by(L2Distance('embedding', match_text))[:5]

                if column.agentic:
                    annotations |= agent_fetch_my_definitions(annotations)

                val = extract_for_query(
                    annotations,
                    column.query,
                    output_type
                )
