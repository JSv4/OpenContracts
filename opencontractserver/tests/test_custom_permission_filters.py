...
class PermissionFilteringTestCase(TestCase):
    def setUp(self):
        ...
        set_permissions_for_obj_to_user(self.user1, self.document1, [PermissionTypes.READ])
        set_permissions_for_obj_to_user(self.user1, self.label1, [PermissionTypes.READ])
        set_permissions_for_obj_to_user(self.user1, self.annotation1, [PermissionTypes.READ])

        set_permissions_for_obj_to_user(self.user2, self.corpus2, [PermissionTypes.READ])
        set_permissions_for_obj_to_user(self.user2, self.document2, [PermissionTypes.READ])
        set_permissions_for_obj_to_user(self.user2, self.label2, [PermissionTypes.READ])
        set_permissions_for_obj_to_user(self.user2, self.annotation2, [PermissionTypes.READ])
    ...
    def test_corpus_permission_filtering(self):
        ...
        self.assertEqual(result1['data']['corpuses']['edges'][0]['node']['title'], 'Corpus 1')

        # Test for user2
        result2 = self.client2.execute(query)
        self.assertEqual(len(result2['data']['corpuses']['edges']), 1)
        self.assertEqual(result2['data']['corpuses']['edges'][0]['node']['title'], 'Corpus 2')
    ...
    def test_document_permission_filtering(self):
        ...
        self.assertEqual(result1['data']['documents']['edges'][0]['node']['title'], 'Document 1')

        # Test for user2
        result2 = self.client2.execute(query)
        self.assertEqual(len(result2['data']['documents']['edges']), 1)
        self.assertEqual(result2['data']['documents']['edges'][0]['node']['title'], 'Document 2')
    ...
    def test_annotation_label_permission_filtering(self):
        ...
        self.assertEqual(result1['data']['annotationLabels']['edges'][0]['node']['text'], 'Label 1')

        # Test for user2
        result2 = self.client2.execute(query)
        self.assertEqual(len(result2['data']['annotationLabels']['edges']), 1)
        self.assertEqual(result2['data']['annotationLabels']['edges'][0]['node']['text'], 'Label 2')
    ...
    def test_annotation_permission_filtering(self):
        ...
        self.assertEqual(result1['data']['annotations']['edges'][0]['node']['document']['title'], 'Document 1')

        # Test for user2
        result2 = self.client2.execute(query)
        self.assertEqual(len(result2['data']['annotations']['edges']), 1)
        self.assertEqual(result2['data']['annotations']['edges'][0]['node']['document']['title'], 'Document 2')
    ...
    def test_nested_permission_filtering(self):
        ...
        result1 = self.client1.execute(query)
        self.assertEqual(len(result1['data']['corpuses']['edges']), 1)
        self.assertEqual(len(result1['data']['corpuses']['edges'][0]['node']['documents']['edges']), 1)
        self.assertEqual(result1['data']['corpuses']['edges'][0]['node']['documents']['edges'][0]['node']['title'],
                         'Document 1')

        # Test for user2
        result2 = self.client2.execute(query)
        self.assertEqual(len(result2['data']['corpuses']['edges']), 1)
        self.assertEqual(len(result2['data']['corpuses']['edges'][0]['node']['documents']['edges']), 1)
        self.assertEqual(result2['data']['corpuses']['edges'][0]['node']['documents']['edges'][0]['node']['title'],
                         'Document 2')
    ...
    def test_permission_change(self):
        ...
        self.assertEqual(len(result1['data']['corpuses']['edges']), 1)
        self.assertEqual(result1['data']['corpuses']['edges'][0]['node']['title'], 'Corpus 2')

        # Grant permission to user2 for corpus1
        set_permissions_for_obj_to_user(self.user2, self.corpus1, [PermissionTypes.READ])

        # Test again for user2
        result2 = self.client2.execute(query)
        self.assertEqual(len(result2['data']['corpuses']['edges']), 2)
        titles = [edge['node']['title'] for edge in result2['data']['corpuses']['edges']]
