...
def cluster_exists():
    ...
    try:
        ...
        print_error(f"Error checking cluster existence: {e}")
        return False
...
def registry_exists():
    ...
    try:
        ...
        print_error(f"Error checking registry existence: {e}")
        return False
...
def setup_cluster():
    ...
    if not cluster_exists():
        ...
        run_command(
            f"ctlptl create cluster kind --registry {REGISTRY_NAME} --name {CLUSTER_NAME}"
        )
    else:
...
def cleanup():
    ...
    try:
        ...
        print_error(f"Error stopping Tilt: {e}")
        pass
